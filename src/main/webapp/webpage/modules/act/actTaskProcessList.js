...
$(document).ready(function() {
	$('#processTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        title: '流程图',
			   formatter:function (value, row, index) {
				   return "<a  href=\"javascript:jp.openDialogView('流程图','${ctx}/act/process/resource/read?procDefId="+row.id+"&resType=image','1000px', '600px')\">"+row.diagramResourceName+"</a>";
			   }
			   ...
			   formatter:function (value, row, index) {
				   return jp.dateFormat(value,"yyyy-MM-dd hh:mm:ss");
					   ...
					   formatter: function operateFormatter(value, row, index) {
						   return [
							   '<a class="start" href="${ctx}/act/task/form?procDefId=' + row.id + '">【启动流程】</a>'].join('');
					   }
                   }
		     ]
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/act/process/import/template';
				  },
  ...
  function deleteAll(){
   ...
   function add(){
	  jp.openDialog('新增发起任务', "${ctx}/act/process/form",'800px', '500px', $('#processTable'));
  }
  function edit(id){//没有权限时，不显示确定按钮
	  ...
	  jp.openDialog('编辑发起任务', "${ctx}/act/process/form?id=" + id,'800px', '500px', $('#processTable'));
	   </shiro:hasPermission>
	  <shiro:lacksPermission name="act:process:edit">
	  jp.openDialogView('查看发起任务', "${ctx}/act/process/form?id=" + id,'800px', '500px', $('#processTable'));
