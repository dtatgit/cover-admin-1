...
$(document).ready(function() {
	$('#actModelTable').bootstrapTable({
		  ...
		  //请求方法
                   ...
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该模型记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/act/model/deleteAll?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        formatter:function(value, row , index){
		        	return "<a href='#'>"+jp.getDictLabel(${fns:toJson(fns:getDictList('act_category'))}, value, "-")+"</a>";
		        }
	  ...
	  $('#actModelTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#edit').prop('disabled', $('#actModelTable').bootstrapTable('getSelections').length!=1);
        });
...
Date.prototype.format = function(format) {
  ...
  function deleteAll(){
   ...
   function add(){
	  jp.openDialog('新增模型', "${ctx}/act/model/create",'800px', '500px', $('#actModelTable'));
  };
