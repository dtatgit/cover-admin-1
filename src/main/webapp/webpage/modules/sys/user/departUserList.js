...
$(document).ready(function() {
	$('#departUserTable').bootstrapTable({
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该系统用户信息记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/sys/departUser/delete?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#departUserTable').bootstrapTable('refresh');
                   	  			jp.success(data.msg);
                   	  		}else{
                   	  			jp.error(data.msg);
                   	  		}
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#departUserTable').bootstrapTable("toggleView");
		}
	  
	  $('#departUserTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#departUserTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#departUserTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/sys/departUser/import/template';
				  },
  ...
  function getIdSelections() {
        return $.map($("#departUserTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该系统用户信息记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/sys/departUser/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#departUserTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
...
function enable(id){
    ...
    jp.confirm('确认要启用吗？', function(){
        jp.loading();
        jp.get("${ctx}/sys/departUser/enable?id=" + id, function(data){
            if(data.success){
                $('#departUserTable').bootstrapTable('refresh');
                jp.success(data.msg);
            }else{
...
function disable(id){
    ...
    jp.confirm('确认要禁用吗？', function(){
        jp.loading();
        jp.get("${ctx}/sys/departUser/disable?id=" + id, function(data){
            if(data.success){
                $('#departUserTable').bootstrapTable('refresh');
                jp.success(data.msg);
            }else{
