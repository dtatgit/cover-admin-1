		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      <tr>
		         ...
		         <td  class="width-15 active" class="active"><label class="pull-right"><font color="red">*</font>岗位名称:</label></td>
		         <td class="width-35"><input id="oldName" name="oldName" type="hidden" value="${role.name}">
					<form:input path="name" htmlEscape="false" maxlength="50" class="form-control"/></td>
		      </tr>
		      <tr>
		         ...
		         <td  class="width-15 active" class="active"><label class="pull-right">岗位类型:</label></td>
		         <td class="width-35"><%--
					...
					<form:select path="roleType" class="form-control ">
						<form:option value="public">公用岗位</form:option>
						<form:option value="sys">系统岗位</form:option>
						<form:option value="user">普通岗位</form:option>
					</form:select>
					<span class="help-inline" title="activiti有3种预定义的组类型：security-role、assignment、user 如果使用Activiti Explorer，需要security-role才能看到manage页签，需要assignment才能claim任务">
						岗位类型（公用岗位：平台包含客户通用岗位、系统岗位：平台内岗位不对外展示、普通岗位：客户自定义岗位名称）</span></td>
		      </tr>
		      <tr>
		         ...
		         <td class="width-35"><form:select path="sysData" class="form-control ">
					...
					<span class="help-inline">“是”代表此数据只有超级管理员能进行修改，“否”则表示拥有岗位修改人员的权限都能进行修改</span></td>
