...
$(document).ready(function() {
	$('#coverBellOperationTable').bootstrapTable({
		  ...
		  //请求方法
                   	...
                   	edit(row.id);
                        ...
                        jp.confirm('确认要删除该井铃操作记录记录吗？', function(){
                       	...
                       	jp.get("${ctx}/cb/equinfo/coverBellOperation/delete?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#coverBellOperationTable').bootstrapTable('refresh');
                   	  			jp.success(data.msg);
                   	  		}else{
                   	  			jp.error(data.msg);
                   	  		}
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jp.getDictLabel(${fns:toJson(fns:getDictList('operation_type'))}, value, "-");
		        }
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#coverBellOperationTable').bootstrapTable("toggleView");
		}
	  
	  $('#coverBellOperationTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
            $('#remove').prop('disabled', ! $('#coverBellOperationTable').bootstrapTable('getSelections').length);
            $('#edit').prop('disabled', $('#coverBellOperationTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/cb/equinfo/coverBellOperation/import/template';
				  },
  ...
  function getIdSelections() {
        return $.map($("#coverBellOperationTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
