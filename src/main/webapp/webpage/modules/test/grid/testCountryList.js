...
$(document).ready(function() {
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/test/grid/testCountry/import/template';
				  },
  ...
  function deleteAll(){
   ...
   function add(){
	  jp.openDialog('新增国家', "${ctx}/test/grid/testCountry/form",'800px', '500px', $('#testCountryTable'));
  }
  function edit(id){//没有权限时，不显示确定按钮
	  ...
	  jp.openDialog('编辑国家', "${ctx}/test/grid/testCountry/form?id=" + id,'800px', '500px', $('#testCountryTable'));
	   </shiro:hasPermission>
	  <shiro:lacksPermission name="test:grid:testCountry:edit">
	  jp.openDialogView('查看国家', "${ctx}/test/grid/testCountry/form?id=" + id,'800px', '500px', $('#testCountryTable'));
