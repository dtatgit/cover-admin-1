...
$(document).ready(function() {
	$('#leaveDialogTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/test/one/leaveDialog/data",
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/test/one/leaveDialog/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该请假表单记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/test/one/leaveDialog/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#leaveDialogTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
