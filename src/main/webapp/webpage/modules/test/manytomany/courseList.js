...
$(document).ready(function() {
	$('#courseTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/test/manytomany/course/form?id=" + row.id;
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/test/manytomany/course/form?id="+row.id+"'>"+value+"</a>";
		         }
	  ...
	  $('#courseTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#edit').prop('disabled', $('#courseTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/test/manytomany/course/import/template';
				  },
  ...
  function edit(){
	  window.location = "${ctx}/test/manytomany/course/form?id=" + getIdSelections();
  }
