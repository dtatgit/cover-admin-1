...
$(document).ready(function() {
	$('#studentCourseTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/test/manytomany/studentCourse/form?id=" + row.id;
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		            if(value == null){
		            	return "<a href='${ctx}/test/manytomany/studentCourse/form?id="+row.id+"'>-</a>";
		            }else{
		            	return "<a href='${ctx}/test/manytomany/studentCourse/form?id="+row.id+"'>"+value+"</a>";
		            }
		        }
	  ...
	  $('#studentCourseTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#edit').prop('disabled', $('#studentCourseTable').bootstrapTable('getSelections').length!=1);
        });
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/test/manytomany/studentCourse/import/template';
				  },
  ...
  function edit(){
	  window.location = "${ctx}/test/manytomany/studentCourse/form?id=" + getIdSelections();
  }
