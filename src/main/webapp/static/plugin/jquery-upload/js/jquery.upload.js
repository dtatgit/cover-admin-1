    ...
    Plugin.prototype = {
        ...
        createImageSection: function($src) {
            var $self = this,
                ...
                //手动添加一个隐藏域存放id
                $hiddenInput2   = $("<input type='hidden' name='" + $fileInput + "_id' value='" + $src + "'>"),
            ...
            //手动添加一个隐藏域存放id
            $hiddenInput2.appendTo($imageSection);
        ...
        createDeleteNode: function($imageSection) {
            ...
            $deleteBox.find(".ggy-confirm-btn").on('click', function(event) {
                ...
                let imgRealPath = $imageSection.find('input[type=hidden]').eq(0).val();
                let fileId = $imageSection.find('input[type=hidden]').eq(1).val();

                jp.post(proPath + "/api/file/deleteimg",{imgRealPath:imgRealPath,fileId:fileId}, function (data) {
        ...
        ajaxUpload: function($file) {
            ...
            $.ajax({
                ...
                success: function($json) {
                    ...
                    $imageSection.find('input[type=hidden]').eq(0).val($json.imgRealPath);
                    //手动添加的隐藏域赋值
                    $imageSection.find('input[type=hidden]').eq(1).val($json.id);
